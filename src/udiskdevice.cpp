...
UdiskDevice::UdiskDevice(QObject *parent) : QObject(parent)
{
    ...
    connect(mProcess,SIGNAL(readyReadStandardOutput()),this,SLOT(deviceStatusChanged()));
...
void UdiskDevice::deviceStatusChanged()
{
...
void UdiskDevice::UpdateDevicesProperties(const QString &result)
{
    ...
    foreach (QString s, list) {
       // qDebug()<<"UpdateDevicesProperties"<<s;
...
//            mListDevice.removeOne(D);
           // setRemoved(s);
             Device  *mDevice =new Device;
            mDevice->setRemoved(s);
            emit deviceRemoved(mDevice);

            qDebug()<<"emit deviceRemoved"<<s;
            delete D;
